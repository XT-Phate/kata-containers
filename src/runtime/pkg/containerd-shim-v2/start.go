...
func startContainer(ctx context.Context, s *service, c *container) (retErr error) {
	...
	if c.stdin != "" || c.stdout != "" || c.stderr != "" {
		...
		go ioCopy(shimLog.WithField("container", c.id), c.exitIOch, c.stdinCloser, nil, nil, tty, stdin, stdout, stderr)
	} else {
...
func startExec(ctx context.Context, s *service, containerID, execID string) (e *exec, retErr error) {
	...
	go ioCopy(shimLog.WithFields(logrus.Fields{
		"container": c.id,
		"exec":      execID,
	}), execs.exitIOch, execs.stdinCloser, execs.stdoutCloser, execs.stderrCloser, tty, stdin, stdout, stderr)
